...
function updateMapFilter(geoJsonLayer, featureLayersMap, filteredMergeIDsSet) {
    ...
    featureLayersMap.forEach((layerSet, merge_id) => {
        const shouldBeVisible = true;//filteredMergeIDsSet.has(merge_id);
...
const mapUpdateStatus = updateMapFilter(mapElements.geoJsonLayer, mapElements.featureLayersMap, null)
```
